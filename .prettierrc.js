module.exports = {
  ...
  trailingComma: "es5", // 对象或数组最后一个元素后是否加逗号，可选值是"none"（不加逗号）, "es5"（在ES5中有效的地方加逗号） 或 "all"（包括函数参数尾部的逗号）
  bracketSpacing: true, // 对象字面量的括号间是否加空格，可选值是true（加空格）或false（不加空格）
  jsxBracketSameLine: true, // 在JSX中的 '>' 符号是否与标签的最后一行末尾对齐，可选值是true（与标签的最后一行末尾对齐）或false（另起一行）
